...
#1. decide on the question:  这个代码实现了简单线性回归的过程，并没有使用 【sklearn】

#2. prepare data
n_samples = 100

#a. Generate 100 random input data (X) - 100 samples (rows),
X = 2 * np.random.rand(n_samples, 1)
'''
indicates the number of columns in the array.
...
#b. Generate corresponding target values (y) with some noise
...
'''
为 X 增加了一列全为1的偏置项(bias term），第一列为 X的原始值，第二列全为 1
X_b 应该是这个样子：
array([[1.        , 1.24896252],
       [1.        , 0.52691165],
       ...
